public class Ascii {

    // ANSI renk sabitleri
    private static final String RESET = "\u001B[0m";
    private static final String RED = "\u001B[31m";
    private static final String GREEN = "\u001B[32m";
    private static final String YELLOW = "\u001B[33m";
    private static final String BLUE = "\u001B[34m";
    private static final String MAGENTA = "\u001B[35m";
    private static final String CYAN = "\u001B[36m";
    private static final String WHITE = "\u001B[37m";

    public static void displayWelcomeMessage() {
        // ÜST ASCII BLOĞU (renkli parçalar halinde)
        String art = BLUE
                + """
                          _________                     .__     _____                      .__               .__
                         /   _____/ ____ _____________  |__|   /  _  \\____________  ______ |  | _____ _______|  |
                         \\_____  \\_/ __ \\\\___   /\\__  \\ |  |  /  /_\\\\  \\_  __ \\\\__  \\ \\____ \\|  | \\\\__  \\\\_  __ \\  |
                         /        \\  ___/ /    /  / __ \\|  | /    |    \\  | \\\\/ __ \\|  |_> >  |__/ __ \\|  | \\\\/  |__
                        /_______  /\\___  >_____ \\(____  /__| \\____|__  /__|  (____  /   __/|____(____  /__|  |____/
                        /        \\/     \\/      \\/     \\/             \\/           \\/|__|             \\/
                        """
                + RESET
                + YELLOW + """
                        ___________                  .__                    ___________
                        \\__    ___/_ __  ____ _____  |  |__ _____    ____   \\__    ___/_ __________ ____
                          |    | |  |  \\/    \\\\__  \\ |  |  \\\\__  \\  /    \\    |    | |  |  \\\\___   // __ \\
                          |    | |  |  /   |  \\/ __ \\|   Y  \\/ __ \\|   |  \\   |    | |  |  //    /\\  ___/
                          |____| |____/|___|  (____  /___|  (____  /___|  /   |____| |____//_____ \\\\___  >
                                            \\/     \\/     \\/     \\/     \\/                       \\/    \\/
                        """ + RESET
                + RED + """
                           _____         .__                    ________
                          /  _  \\ ___.__.|  |__ _____    ____   \\_____  \\   ____   ___________
                         /  /_\\\\  <   |  ||  |  \\\\__  \\  /    \\   /   |   \\ /    \\_/ __ \\_  __ \\
                        /    |    \\\\___  ||   Y  \\/ __ \\|   |  \\ /    |    \\   |  \\  ___/|  | \\/
                        \\____|__  / ____||___|  (____  /___|  / \\_______  /___|  /\\___  >__|
                                \\/\\/          \\/     \\/     \\/          \\/     \\/     \\/
                        """ + RESET
                + GREEN + """
                        __________.__.__         .__     ____  __.     .__
                        \\______   \\__|  | _____  |  |   |    |/ _|____ |  |   ____   ______
                         |    |  _/  |  | \\\\__  \\ |  |   |      <_/ __ \\|  | _/ __ \\ /  ___/
                         |    |   \\  |  |__/ __ \\|  |__ |    |  \\  ___/|  |_\\  ___/ \\___ \\
                         |______  /__|____(____  /____/ |____|__ \\___  >____/\\___  >____  >
                                \\/             \\/               \\/   \\/          \\/     \\/
                        """ + RESET;

        System.out.println(art);

        // ALTTAKİ BİLGİSAYAR ASCII BLOĞU (renkli)
        System.out.println(
                GREEN + "                                                 .------.------.    " + RESET + "\n" +
                        CYAN + "  +-------------+                     ___        " + WHITE + "|      |      |    \n" +
                        CYAN + "  |             |                     \\ /]       " + WHITE + "|      |      |    \n" +
                        CYAN + "  |             |        _           _(_)        " + WHITE + "|      |      |    \n" +
                        CYAN + "  |             |     ___))         [  | \\___    " + WHITE + "|      |      |    \n" +
                        CYAN + "  |             |     ) //o          | |     \\   " + WHITE + "|      |      |    \n" +
                        CYAN + "  |             |  _ (_    >         | |      ]  " + WHITE + "|      |      |    \n" +
                        CYAN + "  |          __ | (O)  \\__<          | | ____/   " + GREEN + "'------'------'    "
                        + RESET + "\n" +
                        BLUE + "  |         /  o| [/] /   \\)        [__|/_                          " + RESET + "\n" +
                        BLUE + "  |             | [\\]|  ( \\         __/___\\_____                    " + RESET + "\n"
                        +
                        BLUE + "  |             | [/]|   \\ \\__  ___|            |                   " + RESET + "\n" +
                        BLUE + "  |             | [\\]|    \\___E/%%/|____________|_____              " + RESET + "\n" +
                        BLUE + "  |             | [/]|=====__   (_____________________)             " + RESET + "\n" +
                        BLUE + "  |             | [\\] \\_____ \\    |                  |              " + RESET + "\n"
                        +
                        BLUE + "  |             | [/========\\ |   |                  |              " + RESET + "\n" +
                        BLUE + "  |             | [\\]     []| |   |                  |              " + RESET + "\n" +
                        BLUE + "  |             | [/]     []| |_  |                  |              " + RESET + "\n" +
                        BLUE + "  |             | [\\]     []|___) |                  |    " + YELLOW + "" + RESET
                        + "\n" +
                        MAGENTA + "====================================================================" + RESET);

        // Grup bilgisi
        System.out.println(RESET + "\nGroup 25 | Sezai Araplarlı | Tunahan Tuze | Ayhan Öner | Bilal Keleş\n");
    }
}
